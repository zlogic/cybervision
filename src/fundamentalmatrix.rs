...
impl FundamentalMatrix {
    ...
    fn optimize_ransac_fundamental_matrix(
        &self,
        match_buckets: &Vec<Vec<Match>>,
        f: &Matrix3<f64>,
    ) -> Option<Matrix3<f64>> {
        let inliers: Vec<Match> = match_buckets
            .par_iter()
            .flat_map(|bucket| {
                let inliers: Vec<Match> = bucket
                    .iter()
                    .filter_map(|m| self.fits_model(&f, m).map(|_err| *m))
                    .collect();
                inliers
            })
            .collect();
    ...
    #[inline]
    pub fn projection_matrix_to_f(p2: &Matrix3x4<f64>) -> Matrix3<f64> {
        let t = p2.column(3);
...
impl LMError {
    fn new(reason: TerminationReason) -> LMError {
        LMError { reason }
    }
}
